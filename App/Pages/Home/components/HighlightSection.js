import {React, TouchableOpacity, GlobalStyles,RFValue, TextBold, Image, ImageBackground, Text, View, useNavigation,} from '../../../Global/Import/Imports';
        ...
        <React.Fragment>
            <ImageBackground style={{padding:5,flex:1,}} resizeMode="cover" source={require('../../../Global/Images/highlight.jpg')}>
                <View style={{flex:1,marginBottom:'5%'}}>
                    <Text style={{fontSize:RFValue(25+1),textAlign:'center',color:'#fff',fontFamily:'Vazir-Bold'}}>
                        قلب قالب شما
                    </Text>
                    <Text style={{fontSize:RFValue(15+1),textAlign:'center',color:'#fff',fontFamily:'Vazir'}}>
        ...
        <React.Fragment>
            <View style={{width:ScreenWidth/2.1,backgroundColor: '#fff', marginTop:10, marginBottom:10, marginRight:2, marginLeft:2, borderRadius:5, elevation: 5}}>
                <TouchableOpacity onPress={() => navigation.navigate('ParentCategory',item.id)} style={{padding:10,backgroundColor:'rgba(0,0,0,.03)',borderBottomWidth:1,borderBottomColor:'rgba(0,0,0,.125)'}} activeOpacity={1}>
                    <TextBold style={{fontSize:RFValue(15+1),textAlign:'center'}}>
                        {item.name}
                    </TextBold>
                </TouchableOpacity>
...
function ItemHighlight ({item}) {
    ...
    return (
        <TouchableOpacity onPress={() => navigation.navigate(item.type,item.id)}
            ...
            <TextBold style={{textAlign:'center',fontSize:RFValue(12+1)}}>
                {item.name}
            </TextBold>
        </TouchableOpacity>
    );
