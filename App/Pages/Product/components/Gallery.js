...
export default function Gallery ({gallery,ParentImage}) {
    const ImageGallery = merge(gallery,[{'id':Math.random(),'image':ParentImage}]).reverse();
    return (
        <View style={{height:ScreenWidth/1.7}}>
            <Slick autoplayTimeout={6} loop={false} showsButtons={false}
                   activeDotColor={'#213854'} autoplay={false} showsPagination={false}>
